...
\usepackage{graphicx}
\usepackage{tikz}
\usetikzlibrary{patterns,intersections,arrows.meta}
...
\begin{keywords}
  ...
  local search;
  sufficient conditions of global extremum;
  heuristic;
...
\begin{figure}
  ...
  \subfloat[Reflection using \textit{Fermat principle}]{
    ...
    \tikz[rotate=-12]{
      ...
      \fill[black]
          (1.5, 0) circle(0.1) coordinate(middle) node[below left]  {$M_i$}
          (1, 1) circle(0.1) coordinate(from) node[above right] {$M_{i-1}$} ++(-1.5,0) node[above] {$C_{i-1}$}
          (4.5, 2) circle(0.1) coordinate(to) node[above right] {$M_{i+1}$} ++(1.5,0) node[below] {$C_{i+1}$};
      \begin{scope}
          \clip (from) circle(1);
      ...
      \draw[thin,-{Latex[length=3mm]}] (middle) to[bend right] (opt);
    }
  }
