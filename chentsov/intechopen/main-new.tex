...
\textbf{%
    Routing processes in constrained problems of sequential traversal of sets
}
\end{center}
...
\begin{abstract}
    ...
    objective function both depending on
    the list of tasks
    is explored.
    The considered setting is focused primarily on engineering applications,
    related to sheet cutting CNC machines;
    other applications are possible,
    for example
    in the problem of minimizing the dose
    loads during dismantling of radiation hazardous elements
    in case of accidents at nuclear power plants.
    The objects to visit are non-empty finite sets -- megacities.
    Problem of tool routing for CNC sheet cutting machines,
    known as
    Cutting Path Problem or Tool Path Problem,
    is considered primary.
    This problem arises during the developing control programs for a CNC machine,
    which set the tool path and a number of technological commands.
    Among formal constraints, the precedence constraints are emphasized,
    which allow to reduce computational complexity and build feasible solutions.
    Dynamic programming (DP) is used as the main research method,
    adopted to take into account the precedence constraints
    and the dependence of objective functions on the list of tasks.
    This type of dependence is of class of very complex constraints
    that determine the admissibility of the route through megacity,
    depending on the path traveled so far.
    With regard to the problem of tool routing in sheet-cutting machines,
    the dependence of the objective function on the list of tasks
    makes it possible to reduce thermal deformations of the material
    during thermal cutting.
    The rigorous mathematical formalization of the constrained routing problem
    and the description of the exact algorithm to solve it
    are provided.
    During the solution process,
    the sequence of tasks execution,
    the trajectory of the process
    and the starting point are optimized.
    Algorithm is implemented as a PC program;
    model examples solved.
\end{abstract}
